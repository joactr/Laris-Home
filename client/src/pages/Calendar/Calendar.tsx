    ...
    return (
        <div className="page">
            <div className="page-header">
                ...
                <button id="calendar-add" className="btn btn-primary btn-sm" onClick={() => openCreate(new Date())}>{t('calendar.newEvent')}</button>
            </div>
            ...
            <div className="week-grid">
                {weekDays.map(day => (
                    <div key={day.toISOString()} className={`day-col ${isSameDay(day, new Date()) ? 'today-highlight' : ''}`}>
                        ...
                        <button
                            style={{ width: '100%', background: 'none', border: 'none', color: 'var(--text2)', fontSize: 18, cursor: 'pointer', padding: '2px 0' }}
                ...
                {events.map(e => (
                    <div key={e.id} className="event-list-item">
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            {e.description && <div style={{ fontSize: 12, color: 'var(--text2)' }}>{e.description}</div>}
                            {e.assigned_name && <div style={{ fontSize: 12, color: 'var(--text2)' }}>{t('calendar.assigned')} {e.assigned_name}</div>}
                        </div>
                        <span className={`badge ${e.category === 'shared' ? 'badge-green' : e.category === 'reminder' ? 'badge-yellow' : 'badge-yellow'}`}>
                        ...
                        <button className="btn-icon" onClick={() => deleteEv(e.id)} style={{ color: 'var(--red)' }}>✕</button>
                    </div>
                ))}
            </div>
                ...
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="modal" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="modal-actions">
                            {editing && (
                                <button className="btn btn-danger btn-sm" onClick={() => { deleteEv(editing.id); setShowModal(false); }}>{t('common.delete')}</button>
                            )}
